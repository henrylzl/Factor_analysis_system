...
def get_matrix_data(name, fpath, codes=None):
    ...
    if codes is not None:
        # 修改：添加错误处理，确保只使用存在于因子数据中的股票代码
        valid_codes = list(set(codes).intersection(set(data.index)))
        if valid_codes:
            data = data.loc[valid_codes, :]
        else:
            data = pd.DataFrame()
        # data = data.loc[codes, :]
    return {name: data}
...
def concat_factors_panel(factors=None, factors_dict=None, codes=None, ind=True, mktcap=True):
    ...
    if codes is not None and factors_dict is not None:
        ...
        for fac, datdf in factors_dict.items():
            # 获取因子数据中实际存在的股票代码
            valid_codes = list(set(codes).intersection(set(datdf.index)))
            if valid_codes:
                filtered_factors_dict[fac] = datdf.loc[valid_codes,:]
        factors_dict = filtered_factors_dict
...
def index_enhance_model(method='l', benchmark='000300.SH', start_date=None, end_date=None, methods=None):
    ...
    valid_codes = list(set(mut_codes).intersection(set(est_stock_rets.index)))
    if valid_codes:
        est_stock_rets = est_stock_rets.loc[valid_codes, mut_dates]
    else:
        est_stock_rets = pd.DataFrame()
...
if __name__ == '__main__':
    ...
    method = input("请选择模型方法（'l'-线性规划; 's'-分层抽样）: ") # 请选择指数增强模型方法
