...
file_path = os.path.join(WORK_PATH, 'factors') #原始因子数据所在目录
...
def get_factor_data(datdf, names=None):
    """根据输入的因子名称将原始因子截面数据分割
    功能：根据输入的因子名称将原始因子截面数据分割
    参数：
        datdf - 原始因子数据DataFrame
        names - 需要处理的因子名称列表(可选)
    返回：
        处理后的因子数据DataFrame, 未处理的因子数据DataFrame
    """
    global info_cols # 基准信息所在列名
    if names:
        try:
            # 将用户输入的因子名称全部转换为小写
...
def factor_industry_comparison(panel_path, matrix_path, sub_dir_name):
    ...
    info_cols_e_zx = [col for col in info_cols if col != 'industry_zx'] # 排除基准列但保留industry_zx
    facs = datdf.columns.difference(set(info_cols_e_zx))
...
if __name__ == '__main__':
    ...
    def input_yes_or_no(tips):
        ...
        参数:
            tips: 提示信息字符串
        返回:
            visualized: bool类型,True代表yes,False代表no
        """
        while True:
            r = input(tips)  # 获取用户输入
            if r in ['y','Y']: visualized = True; break  # 输入y或Y时返回True
            elif r in ['n','N']: visualized = False; break  # 输入n或N时返回False
            else: continue  # 其他输入继续循环    
        return visualized
    ...
    def main():
        ...
        if not os.path.exists(inscmp_path):
            os.makedirs(inscmp_path, exist_ok=True)
        # 对所有横截面进行预处理
