...
# 因子列表目录
quality_path = os.path.join(work_dir, '因子预处理模块','因子数据品质检验', '已预处理')
#测试结果图表存放目录（如无则自动生成）
sf_test_save_path = os.path.join(work_dir, '单因子检验')
if not os.path.exists(sf_test_save_path):
    os.mkdir(sf_test_save_path)
...
def get_factor_names():
    global work_dir
    factor_info = pd.read_csv(os.path.join(quality_path, '可用因子统计.csv'), index_col=[0])
    return factor_info.index.tolist()
...
def get_ind_mktcap_matrix(datdf, ind=True, mktcap=True):
    """
    构建行业和市值中性化矩阵，用于回归时控制行业或市值效应。

    参数:
    datdf : pandas.DataFrame
        包含样本截面数据的表格，应至少包含列:
        - 'MKT_CAP_FLOAT'  浮动市值
        - f'industry_{industry_benchmark}'  行业分类
    ind : bool, default=True
        是否生成行业哑变量矩阵
    mktcap : bool, default=True
        是否生成对数市值变量

    返回:
    pandas.DataFrame
        将对数市值（如选）与行业哑变量（如选）按列拼接后的控制矩阵
    """
    global industry_benchmark

    # 如果需要市值中性，则计算浮动市值的自然对数
    if mktcap:
        # 从输入 DataFrame 中取出市值列并取对数
        lncap = np.log(datdf['MKT_CAP_FLOAT'])
        # 重命名 Series，以便合并后列名清晰
        lncap.name = 'ln_mkt_cap'
    else:
        # 不需要市值中性时，用空 DataFrame 占位
        lncap = pd.DataFrame()

    # 如果需要行业中性，则将行业列转成哑变量
    if ind:
        ind_col = f'industry_{industry_benchmark}'
        # 对行业分类进行 one-hot 编码，生成若干 0/1 列
        ind_dummy_matrix = pd.get_dummies(datdf[ind_col], prefix=ind_col)
    else:
        # 不需要行业中性时，用空 DataFrame 占位
        ind_dummy_matrix = pd.DataFrame()

    # 按列合并对数市值和行业哑变量，作为回归的控制变量矩阵
    return pd.concat([lncap, ind_dummy_matrix], axis=1)

# 计算信息系数（IC）
def get_ic(datdf, fac_name, neutralize=False):
    """
    计算因子信息系数（IC），可选市值和行业中性化。

    参数：
    datdf : pandas.DataFrame
        截面数据，须包含：
        - fac_name 列：当期因子值
        - 'PCT_CHG_NM' 列：下期（下月）收益率
    fac_name : str
        因子列名，用于提取当期因子暴露值
    neutralize : bool, default=False
        是否进行市值和行业中性化（回归残差替换原始因子值）

    返回：
    float
        信息系数（IC），即因子值与下期收益率的皮尔逊相关系数
    """
    # 下期收益率序列（所有股票的下月收益率）
    ...
    if neutralize:
        # 若需中性化，则先构建市值+行业哑变量控制矩阵
        control_matrix = get_ind_mktcap_matrix(datdf)
        # 对原始因子值进行回归，用残差（剔除市值和行业影响后的净因子暴露）替换原始 facdat
        _, _, facdat = regress(facdat, control_matrix)

    # 拼接中性化或未中性化的因子值与下期收益，计算相关系数矩阵
    dat = pd.concat([facdat, pctchgnm], axis=1)
    # 取因子值（第 0 列）与下期收益（第 1 列）的相关系数
    ic = dat.corr().iat[0, 1]
...
def test_yearly(factors=None, start_year=2017, end_year=2025):
    ''' 按年进行检验
...
def panel_to_matrix(factors, factor_path=factor_path, save_path=sf_test_save_path):
    ...
    if not os.path.exists(factor_matrix_path):
        os.makedirs(factor_matrix_path, exist_ok=True)
    else:
        # 检查已存在的因子文件,只处理未保存的因子
    ...
    for f in os.listdir(factor_path):
        ...
        # 只选择存在的因子列
        # available_factors = [factor for factor in factors if factor in datdf.columns]
        # datpanel[date] = datdf[available_factors]
...
def layer_division_backtest(factors):
    global sf_test_save_path
    from factor_synthesis import get_factor
    start_date='2017-01-30' #月频简化回测用不上
    end_date='2025-04-30' #月频简化回测用不上
